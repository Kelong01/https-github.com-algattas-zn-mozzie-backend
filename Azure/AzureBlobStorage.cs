    ...
    public class AzureBlobStorage: IAzureBlobStorage
    {
        ...
        private async Task<CloudBlobContainer> GetContainerAsync()
        {
            ...
            CloudBlobContainer blobContainer =
                blobClient.GetContainerReference(_settings.ContainerName);
        ...
        private async Task<List<AzureBlobItem>> GetBlobListAsync(bool useFlatListing = true)
        {
            ...
            do
            {
                BlobResultSegment resultSegment = await blobContainer.ListBlobsSegmentedAsync("", useFlatListing,
        ...
        /// <returns></returns>
        public async Task<string> UploadSasAsync(string inferenceType, string blobName, string filePath)
        {
            var sas = _settings.SASToken;
            var url =
                $"https://{_settings.StorageAccount}.blob.core.windows.net/{inferenceType}/mobile/{DateTime.Now:yyyyMMdd}/{blobName}?{sas}";
            var cloudBlockBlob = new CloudBlockBlob(new Uri(url));
